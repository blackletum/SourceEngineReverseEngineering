...
static void InstallMainThreadStackHandler()
{
    ...
    {
        ConsolePrint("Failed to install SIGUSR2 stack handler");
        return;
    }
...
static void* AsyncWorkerMain(void* data)
{
    ...
    for (;;)
    {
        ...
        if (last != 0 && (now - last) >= kMainThreadHangSeconds)
        {
            if (!hang_reported)
            {
                ConsolePrint("Watchdog: main thread unresponsive for %d seconds, dumping stack trace to %s", kMainThreadHangSeconds, kStackTraceOutputPath);
...
void InitHangWatchdog()
{
    ...
    if (create_result != 0)
    {
        ConsolePrint("Failed to create async worker thread: %d", create_result);
    ...
    if (detach_result != 0)
    {
        ConsolePrint("Failed to detach async worker thread: %d", detach_result);
    ...
    ConsolePrint("Started detached async worker thread");
}
...
void DeinitHangWatchdog()
{
    if (g_async_thread_started)
    {
        ...
        if (cancel_result != 0)
        {
            ConsolePrint("Failed to cancel async worker thread: %d", cancel_result);
        }
